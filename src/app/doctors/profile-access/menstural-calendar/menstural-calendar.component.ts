...
@Component({
  selector: 'app-menstural-calendar',
  templateUrl: './menstural-calendar.component.html',
  styleUrls: ['./menstural-calendar.component.sass']
})
export class MensturalCalendarComponent implements OnInit {
 ...
 // allStartingDate: any;
    ...
    const data ={
     ...
     this.doctorServiceService.checkAccessdetail(data).subscribe(
      (result)=>{
        ...
        this.myEventData = this.getDataEvent(result.data);
  ...
  // this.myEventData = this.getDataEvent(data);

   setTimeout(() => { 

    //console.log('myEventData', this.myEventData);
  ...
  getDataEvent(data){
    ...
    const userID = {
      "user_id":data.member_id??data.patient_id
    };
...
// FIRST LOOP Logic
      ...
      for(let i= 0; i < 6; i++){
        for(let j=0; j<9 ; j++){
          var result1 = this.addDays((this.calendarDataevents[0].nextDateCount*i) + ((this.oluvationDaysFirst +j)-3), new Date(this.startingDateFirst));
    ...
    // console.log('All Start Date ',  allStartingDate);
    return EventsData;
  }
