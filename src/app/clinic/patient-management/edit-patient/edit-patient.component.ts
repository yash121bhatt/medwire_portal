...
export class EditPatientComponent {
  ...
  ngOnInit(){
   ...
   this.clinicServiceService.patientDetail(data).subscribe((res:any)=>{
    ...
    this.selection = this.patientData.suggested_by_name;
    // console.log('Suggest By ID ',this.patientData.suggested_by_name );
  ...
  doctorList()
{
  ...
  onSubmit() {
    ...
    // console.log('add suggest ,',this.addpatientForm.value.suggested_by);
    ...
    this.form.append('suggested_by_id','344');
    ...
    this.clinicServiceService.updatePatient(this.form).subscribe((res:any)=>{
      if(res.status_code == 200){
        ...
        setTimeout(()=> {
          this.router.navigate(['/clinic/patient-management/patient-list']);
        }, 600 );
  ...
  convertingDate(dateConverting){
    return new Date(dateConverting);
  }
