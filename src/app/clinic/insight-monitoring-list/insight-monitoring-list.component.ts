...
@Component({
  selector: 'app-insight-monitoring-list',
  templateUrl: './insight-monitoring-list.component.html',
  styleUrls: ['./insight-monitoring-list.component.sass']
})
export class InsightMonitoringListComponent implements OnInit {
  ...
  generateReport() {
    ...
    this.PatientServiceService.insightAppointmant(data).subscribe((res:any)=>{
      ...
      this.dataSource3.paginator = this.paginator;
       ...
       var sumLAbTotalBooking: number = res.data.map(a => a.total_booking).reduce(function(a, b)
         {
          return a + b;
         });
      ...
      if(res.status_code === 200){
        this.totalBookingCountLab.push(sumLAbTotalBooking);
  ...
  private Labchart() {
    this.barChartLabOptions = {
      series: [
        {
          name: "Laboratory Total Booking",
          data: this.totalBookingCountLab,
        },
