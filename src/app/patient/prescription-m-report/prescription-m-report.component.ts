...
@Component({
  selector: 'app-prescription-m-report',
  templateUrl: './prescription-m-report.component.html',
  styleUrls: ['./prescription-m-report.component.sass']
})
export class PrescriptionMReportComponent {
...
onSubmit(fData: any,formDirective: FormGroupDirective) {
  ...
  if (this.labreportForm.invalid) {
    ...
    var event = new Date(this.labreportForm.value.dob);

    /* let date = JSON.stringify(event)
    date = date.slice(1,11);
    console.log(date); */
    var ds = this.labreportForm.value.dob;
